...
def quat_from_axis(phi: Tensor, n: Tensor) -> Tensor:
    """
...
def conjugate_quat(q: Tensor) -> Tensor:
    """
...
def quat_vector_mul(q: Tensor, v: Tensor) -> Tensor:
    """
...
def quat_to_3x3_rotation(q: Tensor) -> Tensor:
    """
...
def assemble_4x4_transform(R: Tensor, t: Tensor) -> Tensor:
    """
...
def warp_3d_point(T: Tensor, x: Tensor) -> Tensor:
    """
...
def create_4x4_transform(ex: Tensor, ey: Tensor, translation: Tensor) -> Tensor:
    """
...
def invert_4x4_transform(T: Tensor) -> Tensor:
    """
...
def makeRotX(phi: Tensor) -> Tensor:
    """
...
def calculate_non_chi_transforms() -> Tensor:
    """
...
def calculate_chi_transforms() -> Tensor:
    """
...
def precalculate_rigid_transforms() -> Tensor:
    """
...
def compute_global_transforms(T: Tensor, alpha: Tensor, F: Tensor) -> Tensor:
    """
...
def compute_all_atom_coordinates(T: Tensor, alpha: Tensor, F: Tensor) -> tuple[Tensor, Tensor]:
    """
    ...
    global_transforms = compute_global_transforms(T, alpha, F) # n_res, 8, 4, 4

    atom_local_positions = residue_constants.atom_local_positions.to(device=device,dtype=dtype) # 20, 37, 3
    atom_frame_inds = residue_constants.atom_frame_inds.to(device=device) # 20, 37
    ...
    dim_diff = global_transforms.ndim - atom_frame_inds.ndim # 2
    atom_frame_inds = atom_frame_inds.reshape(atom_frame_inds.shape + (1,) * dim_diff) # n_res, 37, 1, 1
    atom_frame_inds = atom_frame_inds.broadcast_to(atom_frame_inds.shape[:-dim_diff] + global_transforms.shape[-dim_diff:]) # n_res, 37, 4, 4
    atom_frames = torch.gather(global_transforms, dim=-3, index=atom_frame_inds) # n_res, 37, 4, 4

    position_pad = torch.ones(atom_local_positions.shape[:-1]+(1,), device=device, dtype=dtype) # n_res, 37, 1
    padded_local_positions = torch.cat((atom_local_positions, position_pad), dim=-1) # n_res, 37, 4

    global_positions_padded = torch.einsum('...ijk,...ik->...ij', atom_frames, padded_local_positions) # n_res, 37, 4
    global_positions = global_positions_padded[...,:3] # n_res, 37, 3

    atom_mask = residue_constants.atom_mask.to(alpha.device)[F] # n_res, 37
